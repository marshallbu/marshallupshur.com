...
(function( $ ){
  ...
  $.snackbar = function(options) {
    if (isset(options) && options === Object(options)) {
      ...
      if (isset(options.style)) {
        $snackbar.addClass('snackbar').addClass(options.style);
      } else {
        ...
        if ($snackbar.data('animationId1') === animationId1) {
          if (!isset(options.action) || options.action === 'show') {
            $snackbar.addClass('snackbar-opened')
              .promise()
              .done(function() {
                $(this).removeClass('snackbar-closed');
              });
  ...
  $.fn.snackbar = function(action) {
    ...
    if (!this.hasClass('snackbar')) {
      ...
      options.action = action.action || options.action;
      options.content = action.content || '';
      options.style = action.style || '';
      options.timeout = !isNaN(action.timeout) ? action.timeout : 3000;
